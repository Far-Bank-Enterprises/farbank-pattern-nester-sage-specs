...
namespace Farbank.Pattern.Nester.SageSpecs.Server.Services.D365
{
    ...
    {
        //OAuth2AuthenticationHandler handler = new OAuth2AuthenticationHandler handlers;
        ...
        async protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (req.Content != null)
            {
                ...
                if (req.Content is StringContent || this.IsTextBasedContentType(req.Headers) || this.IsTextBasedContentType(req.Content.Headers))
                {
                    ...
                    Debug.WriteLine($"{msg} {string.Join("", result.Cast<char>().Take(1000))}...");
            ...
            if (resp.Content != null)
            {
                ...
                try
                {
                    if (resp.Content is StringContent || this.IsTextBasedContentType(resp.Headers) || this.IsTextBasedContentType(resp.Content.Headers))
                    {
                        ...
                        Debug.WriteLine($"{msg} Content:");
                        Debug.WriteLine($"{msg} {string.Join("", result.Cast<char>().Take(10000))}...");
                        Debug.WriteLine($"{msg} Duration: {end - start}");
                    }
                }
                catch (Exception)
